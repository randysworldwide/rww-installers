...
# Pulls the current version of WinAppUninst.ps1 from the public
# randysworldwide/rww-installers repo and runs it. Do not edit the
# uninstall logic here -- that lives in the repo now. This wrapper only
# handles fetch + run.
...
$branch    = 'main'
$repoPath  = 'Scripts/WinAppUninst.ps1'
$localPath = "$env:ProgramData\Dev\Scripts\WinAppUninst.ps1"
...
try {
    ...
    [Console]::WriteLine("[ERROR] Failed to download uninstall script from GitHub: $($_.Exception.Message)")
    exit 5
}
