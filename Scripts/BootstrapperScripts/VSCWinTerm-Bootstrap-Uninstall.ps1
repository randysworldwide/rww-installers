...
# Pulls the current version of VSCWinTerminalUninst.ps1 from the public
# randysworldwide/rww-installers repo and runs it. Do not edit the
# install/uninstall logic here -- that lives in the repo now. This wrapper
# only handles fetch + run.
...
$branch    = 'main'
$repoPath  = 'Scripts/VSCWinTerminalUninst.ps1'
$localPath = "$env:ProgramData\Dev\Scripts\VSCWinTerminalUninst.ps1"
...
try {
    ...
    [Console]::WriteLine("Fetching latest uninstall script from $url")
    Invoke-WebRequest -Uri $url -OutFile $localPath -UseBasicParsing
} catch {
    [Console]::WriteLine("[BOOTSTRAP] [ERROR] Failed to download uninstall script from GitHub: $($_.Exception.Message)")
    exit 5
}
