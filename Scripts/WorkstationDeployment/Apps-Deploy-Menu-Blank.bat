@echo off
:: --- rww-installers workstation deployment menu (BLANK variant) ---
:: Same as Apps-Deploy-Menu.bat, except every app starts unchecked instead
:: of using the normal fresh-deployment defaults. Meant for going back onto
:: an already-deployed machine to install just a couple of specific apps
:: (e.g. Claude, which installs per-user, so it's often requested one
:: machine/person at a time rather than as part of initial setup).
::
...
:: and runs it elevated with -StartBlank, so the menu on a USB/share is
:: never stale. Do not edit menu/install logic here -- that lives in the repo.
...
powershell -NoProfile -STA -ExecutionPolicy Bypass -File "%LOCAL_PATH%" -StartBlank
