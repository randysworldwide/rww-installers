...
<#
.SYNOPSIS
    Uninstalls Claude via winget. Designed to run elevated on a
    single box (RWW WorkstationDeployment project -- see
    Apps-Deploy-Menu.ps1, Uninstall mode).

.DESCRIPTION
    Repo: randysworldwide/rww-installers
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/ClaudeUninst.ps1

    Counterpart to ClaudeInst.ps1 -- same winget package ID(s), inverse
...
.EXITCODES
    0 = success -- Claude was actually uninstalled this run
    1 = uninstall failed
    3 = not running elevated
    4 = nothing to do -- Claude was not installed
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\ClaudeUninst.log"
)
...
Write-Log "=== Uninstall-Claude starting on $env:COMPUTERNAME ==="
...
if ($anyFailed) {
    Write-Log "=== Uninstall-Claude finished. Overall success: False ==="
    exit 1
} elseif ($allMissing) {
    Write-Log "Nothing was uninstalled -- Claude was not installed." 'WARN'
    Write-Log "=== Uninstall-Claude finished. Nothing to do. ==="
    exit 4
} else {
    Write-Log "=== Uninstall-Claude finished. Overall success: True ==="
    exit 0
}
