...
<#
.SYNOPSIS
    Uninstalls VLC Media Player via winget. Designed to run elevated on a
    single box (RWW WorkstationDeployment project -- see
    Apps-Deploy-Menu.ps1, Uninstall mode).

.DESCRIPTION
    Repo: randysworldwide/rww-installers
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/VLCUninst.ps1

    Counterpart to VLCInst.ps1 -- same winget package ID(s), inverse
...
.EXITCODES
    0 = success -- VLC Media Player was actually uninstalled this run
    1 = uninstall failed
    3 = not running elevated
    4 = nothing to do -- VLC Media Player was not installed
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\VLCUninst.log"
)
...
Write-Log "=== Uninstall-VLC starting on $env:COMPUTERNAME ==="
...
if ($anyFailed) {
    Write-Log "=== Uninstall-VLC finished. Overall success: False ==="
    exit 1
} elseif ($allMissing) {
    Write-Log "Nothing was uninstalled -- VLC Media Player was not installed." 'WARN'
    Write-Log "=== Uninstall-VLC finished. Nothing to do. ==="
    exit 4
} else {
    Write-Log "=== Uninstall-VLC finished. Overall success: True ==="
    exit 0
}
