...
<#
.SYNOPSIS
    Uninstalls Node.js via winget. Designed to run elevated on a
    single box (RWW WorkstationDeployment project -- see
    Apps-Deploy-Menu.ps1, Uninstall mode).

.DESCRIPTION
    Repo: randysworldwide/rww-installers
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/NodeJSUninst.ps1

    Counterpart to NodeJSInst.ps1 -- same winget package ID(s), inverse
...
.EXITCODES
    0 = success -- Node.js was actually uninstalled this run
    1 = uninstall failed
    3 = not running elevated
    4 = nothing to do -- Node.js was not installed
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\NodeJSUninst.log"
)
...
Write-Log "=== Uninstall-NodeJS starting on $env:COMPUTERNAME ==="
...
if ($anyFailed) {
    Write-Log "=== Uninstall-NodeJS finished. Overall success: False ==="
    exit 1
} elseif ($allMissing) {
    Write-Log "Nothing was uninstalled -- Node.js was not installed." 'WARN'
    Write-Log "=== Uninstall-NodeJS finished. Nothing to do. ==="
    exit 4
} else {
    Write-Log "=== Uninstall-NodeJS finished. Overall success: True ==="
    exit 0
}
