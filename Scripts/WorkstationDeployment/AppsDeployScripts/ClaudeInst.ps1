<#
.SYNOPSIS
    Installs Claude (desktop app) machine-wide via winget.
...
.DESCRIPTION
    ...
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/ClaudeInst.ps1

    Standard winget install. No repo-hosted download needed.

    NOTE: this is an Electron app (same install model as Discord/Slack) and
    is per-user by design -- there is no machine-wide/--scope machine
    option, so this deliberately does not attempt one. Whoever is logged in
    and elevated when this runs is whose profile gets the app. If a
    technician runs this while logged in as themselves rather than the
    actual end user, Claude will end up installed for the technician's
    profile, not the employee's -- run this while logged in as (or via
    RunAs for) the actual end user if that matters for this deployment.
...
.EXITCODES
    0 = success -- Claude (desktop app) was actually installed this run
    1 = install failed
    2 = winget could not be resolved on this machine
    3 = not running elevated
    4 = nothing to do -- Claude (desktop app) was already installed (no install action taken)
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\ClaudeInst.log"
)
...
# Main
# ---------------------------------------------------------------------------
Write-Log "=== Install-ClaudeDesktop starting on $env:COMPUTERNAME ==="

if (Test-AppInstalledByRegistry -NameLike 'Claude') {
    Write-Log "Claude (desktop app) already installed. Skipping."
    Write-Log "Nothing was installed -- Claude (desktop app) was already present." 'WARN'
    exit 4
}
...
$code = Install-WithWinget -WingetPath $wingetPath -PackageId 'Anthropic.Claude' -ExtraArgs @()
if ($code -eq 0 -or $code -eq -1978335189 -or (Test-AppInstalledByRegistry -NameLike 'Claude')) {
    Write-Log "Claude (desktop app) installed successfully."
    Write-Log "=== Install-ClaudeDesktop finished. Overall success: True ==="
    exit 0
} else {
    Write-Log "Claude (desktop app) install failed (winget exit code $code)." 'ERROR'
    Write-Log "=== Install-ClaudeDesktop finished. Overall success: False ==="
    exit 1
}
