...
<#
.SYNOPSIS
    Uninstalls 7-Zip via winget. Designed to run elevated on a
    single box (RWW WorkstationDeployment project -- see
    Apps-Deploy-Menu.ps1, Uninstall mode).

.DESCRIPTION
    Repo: randysworldwide/rww-installers
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/7ZipUninst.ps1

    Counterpart to 7ZipInst.ps1 -- same winget package ID(s), inverse
...
.EXITCODES
    0 = success -- 7-Zip was actually uninstalled this run
    1 = uninstall failed
    3 = not running elevated
    4 = nothing to do -- 7-Zip was not installed
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\7ZipUninst.log"
)
...
Write-Log "=== Uninstall-7Zip starting on $env:COMPUTERNAME ==="
...
if ($anyFailed) {
    Write-Log "=== Uninstall-7Zip finished. Overall success: False ==="
    exit 1
} elseif ($allMissing) {
    Write-Log "Nothing was uninstalled -- 7-Zip was not installed." 'WARN'
    Write-Log "=== Uninstall-7Zip finished. Nothing to do. ==="
    exit 4
} else {
    Write-Log "=== Uninstall-7Zip finished. Overall success: True ==="
    exit 0
}
