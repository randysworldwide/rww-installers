...
<#
.SYNOPSIS
    Uninstalls Java (Eclipse Temurin 21) via winget. Designed to run elevated on a
    single box (RWW WorkstationDeployment project -- see
    Apps-Deploy-Menu.ps1, Uninstall mode).

.DESCRIPTION
    Repo: randysworldwide/rww-installers
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/JavaUninst.ps1

    Counterpart to JavaInst.ps1 -- same winget package ID(s), inverse
...
.EXITCODES
    0 = success -- Java (Eclipse Temurin 21) was actually uninstalled this run
    1 = uninstall failed
    3 = not running elevated
    4 = nothing to do -- Java (Eclipse Temurin 21) was not installed
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\JavaUninst.log"
)
...
Write-Log "=== Uninstall-Java starting on $env:COMPUTERNAME ==="
...
$packageIds = @('EclipseAdoptium.Temurin.21.JDK')
...
if ($anyFailed) {
    Write-Log "=== Uninstall-Java finished. Overall success: False ==="
    exit 1
} elseif ($allMissing) {
    Write-Log "Nothing was uninstalled -- Java (Eclipse Temurin 21) was not installed." 'WARN'
    Write-Log "=== Uninstall-Java finished. Nothing to do. ==="
    exit 4
} else {
    Write-Log "=== Uninstall-Java finished. Overall success: True ==="
    exit 0
}
