...
<#
.SYNOPSIS
    Uninstalls Visio LTSC Professional 2021 (volume) via the Office Deployment Tool (ODT).
...
.DESCRIPTION
    ...
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/VisioPro2021Uninst.ps1

    Counterpart to VisioPro2021Inst.ps1 -- same ODT mechanism, inverse
    ...
    targets ONLY this one product (VisioPro2021Volume) instead of removing
    ...
    what VisioPro2021Inst.ps1 installs. If a copy of this product was somehow
    ...
    product IDs -- checked for VisioPro2021Volume specifically. Same check is
...
.EXITCODES
    0 = success -- Visio LTSC Professional 2021 (volume) was actually uninstalled this run
    1 = uninstall failed (setup.exe error, or the product was still
        present afterward)
    2 = could not reach setup.exe on the network share
    3 = not running elevated
    4 = nothing to do -- Visio LTSC Professional 2021 (volume) was not installed
#>
...
$StageDir    = "$env:ProgramData\Dev\AppsDeploy\VisioPro2021Uninstall"
$SharePaths  = @(
...
Write-Log "=== Uninstall-VisioPro2021 starting on $env:COMPUTERNAME ==="

if (-not (Test-ProductInstalled)) {
    Write-Log "Visio LTSC Professional 2021 (volume) ($ProductId) is not installed (not present in ClickToRun ProductReleaseIds). Nothing to do." 'WARN'
    Write-Log "=== Uninstall-VisioPro2021 finished. Nothing to do. ==="
    exit 4
}
...
try {
    ...
    $configPath = Join-Path $StageDir 'configuration-Remove-VisioPro2021Volume.xml'
    $configXml = @"
<Configuration>
  <Remove>
...
if (Test-ProductInstalled) {
    ...
    Write-Log "=== Uninstall-VisioPro2021 finished. Overall success: False ==="
    exit 1
}

Write-Log "Confirmed $ProductId no longer present."
Write-Log "=== Uninstall-VisioPro2021 finished. Overall success: True ==="
exit 0
