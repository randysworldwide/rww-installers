...
<#
.SYNOPSIS
    Uninstalls Dell Command | Update via winget. Designed to run elevated on a
    single box (RWW WorkstationDeployment project -- see
    Apps-Deploy-Menu.ps1, Uninstall mode).

.DESCRIPTION
    Repo: randysworldwide/rww-installers
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/DCUUninst.ps1

    Counterpart to DCUInst.ps1 -- same winget package ID(s), inverse
...
.EXITCODES
    0 = success -- Dell Command | Update was actually uninstalled this run
    1 = uninstall failed
    3 = not running elevated
    4 = nothing to do -- Dell Command | Update was not installed
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\DCUUninst.log"
)
...
Write-Log "=== Uninstall-DCU starting on $env:COMPUTERNAME ==="
...
if ($anyFailed) {
    Write-Log "=== Uninstall-DCU finished. Overall success: False ==="
    exit 1
} elseif ($allMissing) {
    Write-Log "Nothing was uninstalled -- Dell Command | Update was not installed." 'WARN'
    Write-Log "=== Uninstall-DCU finished. Nothing to do. ==="
    exit 4
} else {
    Write-Log "=== Uninstall-DCU finished. Overall success: True ==="
    exit 0
}
