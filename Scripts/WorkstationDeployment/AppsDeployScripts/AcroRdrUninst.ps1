...
<#
.SYNOPSIS
    Uninstalls Adobe Acrobat Reader via winget. Designed to run elevated on a
    single box (RWW WorkstationDeployment project -- see
    Apps-Deploy-Menu.ps1, Uninstall mode).

.DESCRIPTION
    Repo: randysworldwide/rww-installers
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/AcroRdrUninst.ps1

    Counterpart to AcroRdrInst.ps1 -- same winget package ID(s), inverse
    ...
    side): modern unified 64-bit builds of Reader and Acrobat Pro both
    register in Programs & Features as "Adobe Acrobat (64-bit)" -- they
    are literally the same binary, differing only in licensing. Since
    the two are mutually exclusive on a machine anyway (enforced by this
    project's own menu), whichever unified Acrobat is present is the one
    this removes -- winget's correlation against that shared ARP entry
    cannot distinguish a licensed Pro from Reader.
...
.EXITCODES
    0 = success -- Adobe Acrobat Reader was actually uninstalled this run
    1 = uninstall failed
    3 = not running elevated
    4 = nothing to do -- Adobe Acrobat Reader was not installed
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\AcroRdrUninst.log"
)
...
Write-Log "=== Uninstall-AcroRdr starting on $env:COMPUTERNAME ==="
...
$packageIds = @('Adobe.Acrobat.Reader.64-bit')
...
if ($anyFailed) {
    Write-Log "=== Uninstall-AcroRdr finished. Overall success: False ==="
    exit 1
} elseif ($allMissing) {
    Write-Log "Nothing was uninstalled -- Adobe Acrobat Reader was not installed." 'WARN'
    Write-Log "=== Uninstall-AcroRdr finished. Nothing to do. ==="
    exit 4
} else {
    Write-Log "=== Uninstall-AcroRdr finished. Overall success: True ==="
    exit 0
}
