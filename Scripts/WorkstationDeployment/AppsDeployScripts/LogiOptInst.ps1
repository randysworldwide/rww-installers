<#
.SYNOPSIS
    Installs Logi Options+ machine-wide via winget.
...
.DESCRIPTION
    ...
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/LogiOptInst.ps1

    Standard winget install. No repo-hosted download needed.

    NOTE: deliberately NOT passing --scope machine here -- doing so is a
    known failure for this specific package (exit code 1287, see
    microsoft/winget-pkgs#334405). Let winget use its default scope.

    NOTE: this package has also had recurring installer-hash-mismatch
    failures upstream when Logitech updates the installer without bumping
    the winget manifest version (see winget-pkgs#149649, #338590, #114536,
    #172095). If this fails with a hash-mismatch error, that's very likely a
    stale community package, not a problem with this script -- check
    winget-pkgs for an open issue before spending time debugging locally.
...
.EXITCODES
    0 = success -- Logi Options+ was actually installed this run
    1 = install failed
    2 = winget could not be resolved on this machine
    3 = not running elevated
    4 = nothing to do -- Logi Options+ was already installed (no install action taken)
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\LogiOptInst.log"
)
...
# Main
# ---------------------------------------------------------------------------
Write-Log "=== Install-LogiOptionsPlus starting on $env:COMPUTERNAME ==="

if (Test-AppInstalledByRegistry -NameLike 'Logi Options+') {
    Write-Log "Logi Options+ already installed. Skipping."
    Write-Log "Nothing was installed -- Logi Options+ was already present." 'WARN'
    exit 4
}
...
$code = Install-WithWinget -WingetPath $wingetPath -PackageId 'Logitech.OptionsPlus' -ExtraArgs @()
if ($code -eq 0 -or $code -eq -1978335189 -or (Test-AppInstalledByRegistry -NameLike 'Logi Options+')) {
    Write-Log "Logi Options+ installed successfully."
    Write-Log "=== Install-LogiOptionsPlus finished. Overall success: True ==="
    exit 0
} else {
    Write-Log "Logi Options+ install failed (winget exit code $code)." 'ERROR'
    Write-Log "=== Install-LogiOptionsPlus finished. Overall success: False ==="
    exit 1
}
