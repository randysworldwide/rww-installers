<#
.SYNOPSIS
    Installs Node.js (LTS) machine-wide via winget.
...
.DESCRIPTION
    ...
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/NodeJSInst.ps1

    Installs the LTS release line (OpenJS.NodeJS.LTS), not the current/latest
    line -- LTS is the safer default for business machines. Standard winget
    machine-wide install. No repo-hosted download needed.
...
.EXITCODES
    0 = success -- Node.js (LTS) was actually installed this run
    1 = install failed
    2 = winget could not be resolved on this machine
    3 = not running elevated
    4 = nothing to do -- Node.js (LTS) was already installed (no install action taken)
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\NodeJSInst.log"
)
...
# Main
# ---------------------------------------------------------------------------
Write-Log "=== Install-NodeJS starting on $env:COMPUTERNAME ==="

if (Test-AppInstalledByRegistry -NameLike 'Node.js') {
    Write-Log "Node.js (LTS) already installed. Skipping."
    Write-Log "Nothing was installed -- Node.js (LTS) was already present." 'WARN'
    exit 4
}
...
$code = Install-WithWinget -WingetPath $wingetPath -PackageId 'OpenJS.NodeJS.LTS' -ExtraArgs @('--scope','machine')
if ($code -eq 0 -or $code -eq -1978335189 -or (Test-AppInstalledByRegistry -NameLike 'Node.js')) {
    Write-Log "Node.js (LTS) installed successfully."
    Write-Log "=== Install-NodeJS finished. Overall success: True ==="
    exit 0
} else {
    Write-Log "Node.js (LTS) install failed (winget exit code $code)." 'ERROR'
    Write-Log "=== Install-NodeJS finished. Overall success: False ==="
    exit 1
}
