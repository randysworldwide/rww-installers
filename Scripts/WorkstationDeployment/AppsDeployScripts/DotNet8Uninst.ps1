...
<#
.SYNOPSIS
    Uninstalls Microsoft .NET Desktop Runtime 8 via winget. Designed to run elevated on a
    single box (RWW WorkstationDeployment project -- see
    Apps-Deploy-Menu.ps1, Uninstall mode).

.DESCRIPTION
    Repo: randysworldwide/rww-installers
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/DotNet8Uninst.ps1

    Counterpart to DotNet8Inst.ps1 -- same winget package ID(s), inverse
...
.EXITCODES
    0 = success -- Microsoft .NET Desktop Runtime 8 was actually uninstalled this run
    1 = uninstall failed
    3 = not running elevated
    4 = nothing to do -- Microsoft .NET Desktop Runtime 8 was not installed
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\DotNet8Uninst.log"
)
...
Write-Log "=== Uninstall-DotNet8 starting on $env:COMPUTERNAME ==="
...
$packageIds = @('Microsoft.DotNet.DesktopRuntime.8')
...
if ($anyFailed) {
    Write-Log "=== Uninstall-DotNet8 finished. Overall success: False ==="
    exit 1
} elseif ($allMissing) {
    Write-Log "Nothing was uninstalled -- Microsoft .NET Desktop Runtime 8 was not installed." 'WARN'
    Write-Log "=== Uninstall-DotNet8 finished. Nothing to do. ==="
    exit 4
} else {
    Write-Log "=== Uninstall-DotNet8 finished. Overall success: True ==="
    exit 0
}
