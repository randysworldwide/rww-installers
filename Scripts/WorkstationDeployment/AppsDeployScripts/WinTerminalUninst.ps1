...
<#
.SYNOPSIS
    Uninstalls Windows Terminal via winget. Designed to run elevated on a
    single box (RWW WorkstationDeployment project -- see
    Apps-Deploy-Menu.ps1, Uninstall mode).

.DESCRIPTION
    Repo: randysworldwide/rww-installers
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/WinTerminalUninst.ps1

    Counterpart to WinTerminalInst.ps1 -- same winget package ID(s), inverse
...
.EXITCODES
    0 = success -- Windows Terminal was actually uninstalled this run
    1 = uninstall failed
    3 = not running elevated
    4 = nothing to do -- Windows Terminal was not installed
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\WinTerminalUninst.log"
)
...
Write-Log "=== Uninstall-WinTerminal starting on $env:COMPUTERNAME ==="
...
if ($anyFailed) {
    Write-Log "=== Uninstall-WinTerminal finished. Overall success: False ==="
    exit 1
} elseif ($allMissing) {
    Write-Log "Nothing was uninstalled -- Windows Terminal was not installed." 'WARN'
    Write-Log "=== Uninstall-WinTerminal finished. Nothing to do. ==="
    exit 4
} else {
    Write-Log "=== Uninstall-WinTerminal finished. Overall success: True ==="
    exit 0
}
