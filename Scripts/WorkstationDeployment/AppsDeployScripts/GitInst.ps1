<#
.SYNOPSIS
    Installs Git machine-wide via winget.
...
.DESCRIPTION
    ...
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/GitInst.ps1
...
.EXITCODES
    0 = success -- Git was actually installed this run
    1 = install failed
    2 = winget could not be resolved on this machine
    3 = not running elevated
    4 = nothing to do -- Git was already installed (no install action taken)
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\GitInst.log"
)
...
# Main
# ---------------------------------------------------------------------------
Write-Log "=== Install-Git starting on $env:COMPUTERNAME ==="

if (Test-AppInstalledByRegistry -NameLike 'Git') {
    Write-Log "Git already installed. Skipping."
    Write-Log "Nothing was installed -- Git was already present." 'WARN'
    exit 4
}
...
$code = Install-WithWinget -WingetPath $wingetPath -PackageId 'Git.Git' -ExtraArgs @('--scope','machine')
if ($code -eq 0 -or $code -eq -1978335189 -or (Test-AppInstalledByRegistry -NameLike 'Git')) {
    Write-Log "Git installed successfully."
    Write-Log "=== Install-Git finished. Overall success: True ==="
    exit 0
} else {
    Write-Log "Git install failed (winget exit code $code)." 'ERROR'
    Write-Log "=== Install-Git finished. Overall success: False ==="
    exit 1
}
