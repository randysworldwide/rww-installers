<#
.SYNOPSIS
    Installs 7-Zip machine-wide via winget.
...
.DESCRIPTION
    ...
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/7ZipInst.ps1
...
.EXITCODES
    0 = success -- 7-Zip was actually installed this run
    1 = install failed
    2 = winget could not be resolved on this machine
    3 = not running elevated
    4 = nothing to do -- 7-Zip was already installed (no install action taken)
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\7ZipInst.log"
)
...
# Main
# ---------------------------------------------------------------------------
Write-Log "=== Install-SevenZip starting on $env:COMPUTERNAME ==="

if (Test-AppInstalledByRegistry -NameLike '7-Zip') {
    Write-Log "7-Zip already installed. Skipping."
    Write-Log "Nothing was installed -- 7-Zip was already present." 'WARN'
    exit 4
}
...
$code = Install-WithWinget -WingetPath $wingetPath -PackageId '7zip.7zip' -ExtraArgs @('--scope','machine')
if ($code -eq 0 -or $code -eq -1978335189 -or (Test-AppInstalledByRegistry -NameLike '7-Zip')) {
    Write-Log "7-Zip installed successfully."
    Write-Log "=== Install-SevenZip finished. Overall success: True ==="
    exit 0
} else {
    Write-Log "7-Zip install failed (winget exit code $code)." 'ERROR'
    Write-Log "=== Install-SevenZip finished. Overall success: False ==="
    exit 1
}
