...
<#
.SYNOPSIS
    Uninstalls Git via winget. Designed to run elevated on a
    single box (RWW WorkstationDeployment project -- see
    Apps-Deploy-Menu.ps1, Uninstall mode).

.DESCRIPTION
    Repo: randysworldwide/rww-installers
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/GitUninst.ps1

    Counterpart to GitInst.ps1 -- same winget package ID(s), inverse
...
.EXITCODES
    0 = success -- Git was actually uninstalled this run
    1 = uninstall failed
    3 = not running elevated
    4 = nothing to do -- Git was not installed
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\GitUninst.log"
)
...
Write-Log "=== Uninstall-Git starting on $env:COMPUTERNAME ==="
...
if ($anyFailed) {
    Write-Log "=== Uninstall-Git finished. Overall success: False ==="
    exit 1
} elseif ($allMissing) {
    Write-Log "Nothing was uninstalled -- Git was not installed." 'WARN'
    Write-Log "=== Uninstall-Git finished. Nothing to do. ==="
    exit 4
} else {
    Write-Log "=== Uninstall-Git finished. Overall success: True ==="
    exit 0
}
