...
<#
.SYNOPSIS
    Uninstalls Office O365 (Microsoft 365 Apps for business) via the Office Deployment Tool (ODT).
...
.DESCRIPTION
    ...
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/OfficeO365Uninst.ps1

    Counterpart to OfficeO365Inst.ps1 -- same ODT mechanism, inverse
    ...
    targets ONLY this one product (O365BusinessRetail) instead of removing
    ...
    what OfficeO365Inst.ps1 installs. If a copy of this product was somehow
    ...
    product IDs -- checked for O365BusinessRetail specifically. Same check is
...
.EXITCODES
    0 = success -- Office O365 (Microsoft 365 Apps for business) was actually uninstalled this run
    1 = uninstall failed (setup.exe error, or the product was still
        present afterward)
    2 = could not reach setup.exe on the network share
    3 = not running elevated
    4 = nothing to do -- Office O365 (Microsoft 365 Apps for business) was not installed
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\OfficeO365Uninst.log"
)

$ErrorActionPreference = 'Stop'

$StageDir    = "$env:ProgramData\Dev\AppsDeploy\OfficeO365Uninstall"
$SharePaths  = @(
...
Write-Log "=== Uninstall-OfficeO365 starting on $env:COMPUTERNAME ==="

if (-not (Test-ProductInstalled)) {
    Write-Log "Office O365 (Microsoft 365 Apps for business) ($ProductId) is not installed (not present in ClickToRun ProductReleaseIds). Nothing to do." 'WARN'
    Write-Log "=== Uninstall-OfficeO365 finished. Nothing to do. ==="
    exit 4
}
...
try {
    ...
    $configPath = Join-Path $StageDir 'configuration-Remove-O365BusinessRetail.xml'
    $configXml = @"
<Configuration>
  <Remove>
...
if (Test-ProductInstalled) {
    ...
    Write-Log "=== Uninstall-OfficeO365 finished. Overall success: False ==="
    exit 1
}

Write-Log "Confirmed $ProductId no longer present."
Write-Log "=== Uninstall-OfficeO365 finished. Overall success: True ==="
exit 0
