...
<#
.SYNOPSIS
    Uninstalls GitHub CLI via winget. Designed to run elevated on a
    single box (RWW WorkstationDeployment project -- see
    Apps-Deploy-Menu.ps1, Uninstall mode).

.DESCRIPTION
    Repo: randysworldwide/rww-installers
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/GHCliUninst.ps1

    Counterpart to GHCliInst.ps1 -- same winget package ID(s), inverse
...
.EXITCODES
    0 = success -- GitHub CLI was actually uninstalled this run
    1 = uninstall failed
    3 = not running elevated
    4 = nothing to do -- GitHub CLI was not installed
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\GHCliUninst.log"
)
...
Write-Log "=== Uninstall-GHCli starting on $env:COMPUTERNAME ==="
...
if ($anyFailed) {
    Write-Log "=== Uninstall-GHCli finished. Overall success: False ==="
    exit 1
} elseif ($allMissing) {
    Write-Log "Nothing was uninstalled -- GitHub CLI was not installed." 'WARN'
    Write-Log "=== Uninstall-GHCli finished. Nothing to do. ==="
    exit 4
} else {
    Write-Log "=== Uninstall-GHCli finished. Overall success: True ==="
    exit 0
}
