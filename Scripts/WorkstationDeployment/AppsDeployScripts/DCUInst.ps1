<#
.SYNOPSIS
    Installs Dell Command | Update machine-wide via winget.
...
.DESCRIPTION
    ...
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/DCUInst.ps1

    Standard winget machine-wide install. winget pulls the installer straight
    from Dell (dl.dell.com) -- no repo-hosted download needed.

    NOTE: Dell's own CDN link behind this winget package has intermittently
    404'd for some versions (see microsoft/winget-pkgs#266851). If this
    fails with a download error rather than an install error, check whether
    it's a known Dell-side outage before assuming the script is broken.

    Only meaningful on Dell hardware -- fine to run anywhere, it just won't
    do anything useful on non-Dell machines.
...
.EXITCODES
    0 = success -- Dell Command | Update was actually installed this run
    1 = install failed
    2 = winget could not be resolved on this machine
    3 = not running elevated
    4 = nothing to do -- Dell Command | Update was already installed (no install action taken)
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\DCUInst.log"
)
...
# Main
# ---------------------------------------------------------------------------
Write-Log "=== Install-DellCommandUpdate starting on $env:COMPUTERNAME ==="

if (Test-AppInstalledByRegistry -NameLike 'Dell Command | Update') {
    Write-Log "Dell Command | Update already installed. Skipping."
    Write-Log "Nothing was installed -- Dell Command | Update was already present." 'WARN'
    exit 4
}
...
$code = Install-WithWinget -WingetPath $wingetPath -PackageId 'Dell.CommandUpdate' -ExtraArgs @('--scope','machine')
if ($code -eq 0 -or $code -eq -1978335189 -or (Test-AppInstalledByRegistry -NameLike 'Dell Command | Update')) {
    Write-Log "Dell Command | Update installed successfully."
    Write-Log "=== Install-DellCommandUpdate finished. Overall success: True ==="
    exit 0
} else {
    Write-Log "Dell Command | Update install failed (winget exit code $code)." 'ERROR'
    Write-Log "=== Install-DellCommandUpdate finished. Overall success: False ==="
    exit 1
}
