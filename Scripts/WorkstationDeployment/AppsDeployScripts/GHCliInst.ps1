<#
.SYNOPSIS
    Installs GitHub CLI machine-wide via winget.
...
.DESCRIPTION
    ...
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/GHCliInst.ps1
...
.EXITCODES
    0 = success -- GitHub CLI was actually installed this run
    1 = install failed
    2 = winget could not be resolved on this machine
    3 = not running elevated
    4 = nothing to do -- GitHub CLI was already installed (no install action taken)
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\GHCliInst.log"
)
...
# Main
# ---------------------------------------------------------------------------
Write-Log "=== Install-GitHubCLI starting on $env:COMPUTERNAME ==="

if (Test-AppInstalledByRegistry -NameLike 'GitHub CLI') {
    Write-Log "GitHub CLI already installed. Skipping."
    Write-Log "Nothing was installed -- GitHub CLI was already present." 'WARN'
    exit 4
}
...
$code = Install-WithWinget -WingetPath $wingetPath -PackageId 'GitHub.cli' -ExtraArgs @('--scope','machine')
if ($code -eq 0 -or $code -eq -1978335189 -or (Test-AppInstalledByRegistry -NameLike 'GitHub CLI')) {
    Write-Log "GitHub CLI installed successfully."
    Write-Log "=== Install-GitHubCLI finished. Overall success: True ==="
    exit 0
} else {
    Write-Log "GitHub CLI install failed (winget exit code $code)." 'ERROR'
    Write-Log "=== Install-GitHubCLI finished. Overall success: False ==="
    exit 1
}
