...
<#
.SYNOPSIS
    Installs Visio LTSC Professional 2021 (volume-licensed) via the Office
...
.DESCRIPTION
    ...
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/VisioPro2021Inst.ps1
    ...
    This script generates its own minimal config requesting ONLY Visio,
    independent of Office2021Inst.ps1 and ProjectPro2021Inst.ps1. Per
    ...
    VisioPro2021Volume under Channel="PerpetualVL2021".
...
.EXITCODES
    0 = success -- Visio was actually installed this run
    1 = setup.exe reported a non-zero exit code
    2 = could not reach setup.exe on the share, or couldn't write the local config
    3 = not running elevated
    4 = nothing to do -- a matching Visio install was already present
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\VisioPro2021Inst.log"
)

$ErrorActionPreference = 'Stop'

$StageDir   = "$env:ProgramData\Dev\AppsDeploy\VisioPro2021"
$SharePaths = @(
...
function Test-VisioInstalled {
    ...
    foreach ($hive in $hives) {
        $match = Get-ItemProperty -Path $hive -ErrorAction SilentlyContinue |
            Where-Object { $_.DisplayName -like '*Microsoft Visio*' }
        if ($match) { return $true }
    }
    return $false
}

Write-Log "=== Install-VisioPro2021 starting on $env:COMPUTERNAME ==="

if (Test-VisioInstalled) {
    Write-Log "A matching Visio install is already present (matched on DisplayName). Skipping."
    Write-Log "Nothing was installed -- Visio was already present." 'WARN'
    exit 4
}
...
try {
    ...
    $localConfig = Join-Path $StageDir 'configuration-VisioPro2021.xml'
    ...
    Write-Log "Writing generated Visio-only config to $localConfig"
...
if ($finalCode -ne 0) {
    ...
    Write-Log "=== Install-VisioPro2021 finished. Overall success: False ==="
    exit 1
}

if (Test-VisioInstalled) {
    Write-Log "Visio confirmed present after install."
    Write-Log "=== Install-VisioPro2021 finished. Overall success: True ==="
    exit 0
} else {
    Write-Log "setup.exe exited 0 but no matching install was found in the uninstall registry afterward." 'WARN'
    Write-Log "=== Install-VisioPro2021 finished. Overall success: True (unverified by registry) ==="
    exit 0
}
