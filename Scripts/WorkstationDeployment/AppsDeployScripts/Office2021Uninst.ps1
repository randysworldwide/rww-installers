...
<#
.SYNOPSIS
    Uninstalls Office Suite 2021 (Professional Plus, volume) via the Office Deployment Tool (ODT).
...
.DESCRIPTION
    ...
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/Office2021Uninst.ps1

    Counterpart to Office2021Inst.ps1 -- same ODT mechanism, inverse
    ...
    targets ONLY this one product (ProPlus2021Volume) instead of removing
    ...
    what Office2021Inst.ps1 installs. If a copy of this product was somehow
    ...
    product IDs -- checked for ProPlus2021Volume specifically. Same check is
...
.EXITCODES
    0 = success -- Office Suite 2021 (Professional Plus, volume) was actually uninstalled this run
    1 = uninstall failed (setup.exe error, or the product was still
        present afterward)
    2 = could not reach setup.exe on the network share
    3 = not running elevated
    4 = nothing to do -- Office Suite 2021 (Professional Plus, volume) was not installed
#>
...
$StageDir    = "$env:ProgramData\Dev\AppsDeploy\Office2021Uninstall"
$SharePaths  = @(
...
Write-Log "=== Uninstall-Office2021 starting on $env:COMPUTERNAME ==="

if (-not (Test-ProductInstalled)) {
    Write-Log "Office Suite 2021 (Professional Plus, volume) ($ProductId) is not installed (not present in ClickToRun ProductReleaseIds). Nothing to do." 'WARN'
    Write-Log "=== Uninstall-Office2021 finished. Nothing to do. ==="
    exit 4
}
...
try {
    ...
    $configPath = Join-Path $StageDir 'configuration-Remove-ProPlus2021Volume.xml'
    $configXml = @"
<Configuration>
  <Remove>
...
if (Test-ProductInstalled) {
    ...
    Write-Log "=== Uninstall-Office2021 finished. Overall success: False ==="
    exit 1
}

Write-Log "Confirmed $ProductId no longer present."
Write-Log "=== Uninstall-Office2021 finished. Overall success: True ==="
exit 0
