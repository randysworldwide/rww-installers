...
<#
.SYNOPSIS
    Uninstalls Microsoft Visual C++ Redistributables via winget. Designed to run elevated on a
    single box (RWW WorkstationDeployment project -- see
    Apps-Deploy-Menu.ps1, Uninstall mode).

.DESCRIPTION
    Repo: randysworldwide/rww-installers
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/VCRedistUninst.ps1

    Counterpart to VCRedistInst.ps1 -- same winget package ID(s), inverse
...
.EXITCODES
    0 = success -- Microsoft Visual C++ Redistributables was actually uninstalled this run
    1 = uninstall failed
    3 = not running elevated
    4 = nothing to do -- Microsoft Visual C++ Redistributables was not installed
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\VCRedistUninst.log"
)
...
Write-Log "=== Uninstall-VCRedist starting on $env:COMPUTERNAME ==="
...
$packageIds = @('Microsoft.VCRedist.2015+.x64', 'Microsoft.VCRedist.2015+.x86')
...
if ($anyFailed) {
    Write-Log "=== Uninstall-VCRedist finished. Overall success: False ==="
    exit 1
} elseif ($allMissing) {
    Write-Log "Nothing was uninstalled -- Microsoft Visual C++ Redistributables was not installed." 'WARN'
    Write-Log "=== Uninstall-VCRedist finished. Nothing to do. ==="
    exit 4
} else {
    Write-Log "=== Uninstall-VCRedist finished. Overall success: True ==="
    exit 0
}
