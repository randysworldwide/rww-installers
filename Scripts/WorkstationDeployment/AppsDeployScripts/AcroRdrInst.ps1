<#
.SYNOPSIS
    Installs Adobe Acrobat Reader machine-wide via winget.
...
.DESCRIPTION
    ...
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/AcroRdrInst.ps1

    Free Adobe Acrobat Reader only -- NOT Acrobat Pro. Reader and Pro are
    mutually exclusive in the deployment menu (selecting one deselects and
    disables the other) since modern, 64-bit-unified Acrobat Reader and
    Acrobat Pro cannot coexist on the same machine -- installing one
    replaces the other.
    ...
    has it is a no-op. Matches broadly on "Adobe Acrobat" rather than
    "Adobe Acrobat Reader" specifically -- the actual registered
    DisplayName for winget-installed Reader turned out to be "Adobe
    Acrobat (64-bit)" in testing, not "Adobe Acrobat Reader". Given Reader
    and Pro can't coexist anyway, "any Adobe Acrobat family product
    already present" is the correct signal to skip.
...
.EXITCODES
    0 = success -- Adobe Acrobat Reader was actually installed this run
    1 = install failed
    2 = winget could not be resolved on this machine
    3 = not running elevated
    4 = nothing to do -- Adobe Acrobat Reader was already installed (no install action taken)
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\AcroRdrInst.log"
)
...
# Main
# ---------------------------------------------------------------------------
Write-Log "=== Install-AcrobatReader starting on $env:COMPUTERNAME ==="

if (Test-AppInstalledByRegistry -NameLike 'Adobe Acrobat') {
    Write-Log "Adobe Acrobat Reader already installed. Skipping."
    Write-Log "Nothing was installed -- Adobe Acrobat Reader was already present." 'WARN'
    exit 4
}
...
$code = Install-WithWinget -WingetPath $wingetPath -PackageId 'Adobe.Acrobat.Reader.64-bit' -ExtraArgs @('--scope','machine')
if ($code -eq 0 -or $code -eq -1978335189 -or (Test-AppInstalledByRegistry -NameLike 'Adobe Acrobat')) {
    Write-Log "Adobe Acrobat Reader installed successfully."
    Write-Log "=== Install-AcrobatReader finished. Overall success: True ==="
    exit 0
} else {
    Write-Log "Adobe Acrobat Reader install failed (winget exit code $code)." 'ERROR'
    Write-Log "=== Install-AcrobatReader finished. Overall success: False ==="
    exit 1
}
