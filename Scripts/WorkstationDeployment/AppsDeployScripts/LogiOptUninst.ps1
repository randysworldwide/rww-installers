...
<#
.SYNOPSIS
    Uninstalls Logi Options+ via winget. Designed to run elevated on a
    single box (RWW WorkstationDeployment project -- see
    Apps-Deploy-Menu.ps1, Uninstall mode).

.DESCRIPTION
    Repo: randysworldwide/rww-installers
    Path: Scripts/WorkstationDeployment/AppsDeployScripts/LogiOptUninst.ps1

    Counterpart to LogiOptInst.ps1 -- same winget package ID(s), inverse
...
.EXITCODES
    0 = success -- Logi Options+ was actually uninstalled this run
    1 = uninstall failed
    3 = not running elevated
    4 = nothing to do -- Logi Options+ was not installed
#>

[CmdletBinding()]
param(
    [string]$LogPath = "$env:ProgramData\Dev\AppsDeploy\Logs\LogiOptUninst.log"
)
...
Write-Log "=== Uninstall-LogiOpt starting on $env:COMPUTERNAME ==="
...
$packageIds = @('Logitech.OptionsPlus')
...
if ($anyFailed) {
    Write-Log "=== Uninstall-LogiOpt finished. Overall success: False ==="
    exit 1
} elseif ($allMissing) {
    Write-Log "Nothing was uninstalled -- Logi Options+ was not installed." 'WARN'
    Write-Log "=== Uninstall-LogiOpt finished. Nothing to do. ==="
    exit 4
} else {
    Write-Log "=== Uninstall-LogiOpt finished. Overall success: True ==="
    exit 0
}
